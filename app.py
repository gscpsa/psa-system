...
def extract_card_items_from_pdf(pdf_path):
    ...
    for page_index, info in enumerate(page_infos):
        ...
        for block_index, cert_block, cert_number in cert_blocks:
            ...
            # Do NOT trust embedded PDF image order/content. Some PSA PDFs expose image
            # objects in an order that does not match the visible card rows.
            # Instead, crop the visible thumbnail from the left column at this exact row.
            row_anchor_y = cert_y
            if grade_y is not None:
                # Use the midpoint between grade and cert so the crop is centered on the full card row.
                row_anchor_y = (grade_y + cert_y) / 2

            image_data = crop_thumbnail_from_page(info["page"], row_anchor_y)

            # Fallback only: closest unused embedded left-thumbnail object.
            if not image_data:
                candidates = []
...
def portal_orders():
    ...
    <style>
        ...
        @media (max-width:700px) {
            body.portal-body .container { padding:16px 12px 32px; }
            .portal-results-head h1 { font-size:25px; }
        }
    </style>
    <div class="portal-results-head">
        <h1>Your PSA Orders</h1>
        <p>Track your submission status, pickup readiness, text alerts, and buyback offers.</p>
    </div>
    """

    grouped = {}

    def portal_values_text(data):
        try:
            return " ".join(str(v) for v in (data or {}).values() if v is not None)
        except Exception:
            return ""

    def portal_customer_name(data):
        return str(get_field(data, [
            "Customer Name", "Customer", "Name", "Full Name", "Billing Name", "Client", "Bill To"
        ]) or "")

    def portal_customer_phone(data):
        return normalize_phone(get_field(data, [
            "Contact Info", "Phone", "Phone Number", "Customer Phone", "Customer Contact",
            "Contact", "Mobile", "Mobile Phone", "Cell", "Cell Phone", "Billing Phone", "Telephone"
        ]))

    def portal_submission(data):
        return normalize_submission(get_field(data, [
            "Submission #", "Submission Number", "Submission", "Order #", "Order Number",
            "PSA Order #", "PSA Order Number", "PSA Submission #"
        ]))

    for r in rows:
        data = r[0] or {}
        values_text = portal_values_text(data).lower()
        name = portal_customer_name(data).lower()
        contact = portal_customer_phone(data)
        all_digits = normalize_phone(portal_values_text(data))
        sub = portal_submission(data)

        phone_match = False
        if phone and contact and (phone in contact or contact in phone):
            phone_match = True
        elif phone and all_digits and (phone in all_digits or (len(phone) >= 7 and phone[-7:] in all_digits)):
            phone_match = True

        name_match = bool(last) and (last in name or last in values_text)
    ...
    for status_option in statuses_available:
        html += f"<option value='{html_escape(status_option)}' {selected(status_option, selected_status)}>{html_escape(status_option)}</option>"
    ...
    if not filtered_grouped:
        ...
        html += "<a href='/portal/orders?view=all&status=all'>Show all orders</a>"
        return page(html, mode="portal")
    ...
    for sub, grouped_values in filtered_grouped.items():
        ...
        cards = get_field(data, ["# Of Cards", "# of Cards", "Cards", "Card Count", "Items"])
        service = clean_service_display(get_field(data, ["Service Type", "Service", "Service Level"]));
        date = get_dropoff_date(data)
        arrived_completed_raw = get_field(data, ["Arrived / Completed", "Arrived", "Arrived at PSA"])
        arrived_completed_data = parse_arrived_completed_value(arrived_completed_raw)
        arrived_completed = arrived_completed_data["arrived"] or strip_arrived_at_psa_prefix(arrived_completed_data["display"])
        estimated_completion = get_field(data, ["Estimated Completion Date", "Estimated Completion"]) or arrived_completed_data["estimated"]
        ...
        if buyback_rows:
            ...
            <details class="buyback-collapsible">
                ...
                <div class="buyback-inner">
                    ...
                    <form method="post" action="/portal/sell_interest">
                        <input type="hidden" name="submission_number" value="{html_escape(sub)}">
                        <div class="card-grid">
            """
            ...
            for row in buyback_rows:
                ...
                buyback_status = row[14] if len(row) > 14 else "New"
                ...
                <div class="buy-card">
                    {img_html}
                    <div class="cert">Certification #: {html_escape(cert_number)}</div>
                    <div><b>Type:</b> {html_escape(card_type)}</div>
                    <div>{html_escape(item_details)}</div>
                    <div><b>Grade:</b> {html_escape(grade)}</div>
                    {offer_display}
                    <label class="sell-check"><input type="checkbox" name="cert" value="{html_escape(cert_number)}" {checked}> Interested in selling</label>
                </div>
                """
        ...
        <form method="post" action="/portal/sms_preferences">
            ...
            <label class="sell-check"><input type="radio" name="sms_mode" value="none" {none_checked}> No text messages</label>
            <label class="sell-check"><input type="radio" name="sms_mode" value="pickup" {pickup_checked}> Text me when this submission is ready for pickup</label>
            <label class="sell-check"><input type="radio" name="sms_mode" value="all" {all_checked}> Text me for every PSA status change on this submission</label>
        ...
        html += f"""
        <div class="card">
            <h3>{html_escape(customer_name)}</h3>
            <p><b>Submission #:</b> {html_escape(sub)}</p>
            <p><b>Status:</b> <span class="status">{html_escape(display_status_label)}</span></p>
            <p><b>Arrived at PSA:</b> {html_escape(arrived_completed)}</p>
            <p><b>Estimated Completion Date:</b> {html_escape(estimated_completion)}</p>
            <p><b>Cards:</b> {html_escape(cards)}</p>
            <p><b>Service:</b> {html_escape(service)}</p>
            <p><b>Customer Drop-Off Date:</b> {html_escape(date)}</p>
