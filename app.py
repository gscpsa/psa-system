...
import pandas as pd
import psycopg2
import os, io, json, re, traceback, base64, smtplib, secrets, hmac, time, threading
...
def _inject_csrf_into_forms(content):
    safe_token = html_escape(_get_csrf_token())
    pattern = re.compile(
        r'(<form\b[^>]*\bmethod\s*=\s*["\']post["\'][^>]*>)',
        re.IGNORECASE
    )
    return pattern.sub(
        lambda match: match.group(1) + f'<input type="hidden" name="_csrf_token" value="{safe_token}">',
        str(content or "")
    )
...
def _record_failed_attempt(store, key):
    with _rate_limit_lock:
...
def _admin_password_matches(candidate):
...
def _phones_match(entered, stored):
    entered = normalize_phone(entered)
    stored = normalize_phone(stored)
    if not entered or not stored:
        return False
    if len(entered) >= 10 and len(stored) >= 10:
        return entered[-10:] == stored[-10:]
    return entered == stored


def _last_name_matches(entered, full_name):
    entered = re.sub(r"\s+", " ", str(entered or "").strip().lower())
    full_name = re.sub(r"\s+", " ", str(full_name or "").strip().lower())
    if not entered or not full_name:
        return False
    return full_name == entered or full_name.endswith(" " + entered) or full_name.startswith(entered + ",")
...
def setup():
    ...
    try:
        ...
        app.logger.exception("Database initialization check failed.")
...
def security_headers(response):
    ...
    if request.path.startswith("/admin") or request.path.startswith("/portal/orders"):
...
def upload_too_large(e):
    ...
    <div class="card">
        ...
        <p>The uploaded file exceeds the 50 MB safety limit.</p>
...
def error_handler(e):
    ...
    try:
        ...
        <div class="card">
            ...
            <p>The app hit an internal error. Please try again or check the Railway logs.</p>
            <a href="/admin">Back to Admin</a>
        </div>
        """), 500
    except Exception:
...
def save_row(sub, raw):
    ...
    existing_raw = existing[1] if existing and existing[1] else {}

    # Spreadsheet refreshes must never erase PSA-PDF-owned link metadata.
    merged_raw = dict(raw or {})
    for protected_field in ("PSA Order URL", "PSA Order #"):
        if protected_field in existing_raw:
            merged_raw[protected_field] = existing_raw[protected_field]
...
def page(content, mode="admin"):
    content = _inject_csrf_into_forms(content)
...
<meta name='viewport' content='width=device-width, initial-scale=1.0'>
    ...
    <body class="{mode}-body">
        <div class="topbar">
            <div class="brand"><img src="data:image/png;base64,{HEADER_LOGO_B64}" alt="Giant Sports Cards"> <span>PSA Submission Tracker</span></div>
...
def admin_login():
    if request.method == "POST":
        ...
        if _rate_limited(_login_attempts, client_key, 10, 15 * 60):
            return page("<div class='card'>Too many login attempts. Please wait 15 minutes and try again.</div>"), 429
        if _admin_password_matches(request.form.get("password")):
            _clear_attempts(_login_attempts, client_key)
        ...
        _record_failed_attempt(_login_attempts, client_key)
...
@app.route("/admin/clear_submissions", methods=["GET", "POST"])
@admin_required
def clear_submissions():
    if request.method == "GET":
        return page("""
        <div class="card">
            <h2>Clear All Submission Data?</h2>
            <p>This permanently deletes all submission records. This action cannot be undone.</p>
            <form method="post">
                <button type="submit">Confirm Delete All Submissions</button>
            </form>
    ...
    try:
        ...
        app.logger.exception("Error clearing submission data")
        return page("""
        <div class="card">
            <h2>Error Clearing Submission Data</h2>
            <p>An internal error occurred. Check the Railway logs for details.</p>
            <a class="btn" href="/admin">Back to Admin</a>
        </div>
        """)
...
def admin_search():
    ...
    <h2>Admin Search</h2>
    <form>
        <input name="q" value="{html_escape(q)}" placeholder="Search name, phone, submission, status">
    ...
    if request.method == "POST":
        try:
            ...
            <div class="card">
                ...
                <p>The file could not be processed. Check the Railway logs for details.</p>
...
def admin_upload_psa():
    if request.method == "POST":
        try:
            ...
            if not filename.endswith(".pdf") or not _looks_like_pdf(file):
                return page(f"""
                <div class="card">
            ...
            app.logger.exception("PSA status PDF upload failed")
            return page("""
            <div class="card">
...
def admin_upload_cards():
    if request.method == "POST":
        try:
            ...
            if filename.endswith(".pdf") and not _looks_like_pdf(file):
                return page("""
                <div class="card">
                    <h2>Invalid PDF File</h2>
                    <p>The uploaded file does not appear to be a valid PDF.</p>
                    <a href="/admin/upload_cards">Back to Card Upload</a>
                </div>
                """)
            ...
            app.logger.exception("Card PDF upload failed")
            return page("""
            <div class="card">
...
def portal_sms_preferences():
    ...
    for sub_value, data in rows:
        ...
        phone_match = _phones_match(phone, contact)
        name_match = _last_name_matches(last, name)
...
def portal():
    if request.method == "POST":
        client_key = _client_ip()
        if _rate_limited(_portal_attempts, client_key, 30, 10 * 60):
            return page("<div class='card'>Too many lookup attempts. Please wait a few minutes and try again.</div>", mode="portal"), 429
        _record_failed_attempt(_portal_attempts, client_key)
