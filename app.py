...
def page(content):
    ...
    .card {{background:white;padding:18px;margin-bottom:15px;border-radius:10px}}
    .status {{font-weight:bold;color:#2563eb}}
    .bar {{display:flex;gap:6px;margin-top:10px}}
    .step {{padding:6px 10px;border-radius:20px;background:#e5e7eb}}
    .done {{background:#bfdbfe}}
...
def portal():
    ...
    return page("<form method='post'><input name='phone'><input name='last'><button>View</button></form>")
...
def orders():
    ...
    html="<h2>Orders</h2>"
    for sub,status in rows:
        html+=f"<div class='card'>{sub} - {status}{status_bar(status)}</div>"
