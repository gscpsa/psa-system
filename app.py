...
def save_row(sub, raw):
    ...
    # Remove any Excel-provided status keys (avoid confusion with PSA Status)
    for k in list(raw.keys()):
        if "status" in str(k).lower():
            del raw[k]

    if picked_up:
        # Lock to Picked Up (terminal)
        cur.execute("""
        ...
        # Do not overwrite existing non-null status; keep current PSA progression
        cur.execute("""
        INSERT INTO submissions (submission_number, status, raw_data)
        VALUES (%s, %s, %s)
        ON CONFLICT (submission_number)
...
# =========================
# ADMIN PDF UPLOAD — ADVANCED TABLE PARSER + FALLBACK
# =========================
    ...
    if request.method == "POST":
        ...
        with pdfplumber.open(temp.name) as pdf:
            for page in pdf.pages:
                ...
                # --- TEXT PARSER (fallback for pages where tables fail)
                text = page.extract_text() or ""
                entries = re.findall(r"(Sub\s*#\s*\d+.*?)(?=Sub\s*#\s*\d+|$)", text, re.DOTALL)

                for entry in entries:
                    sub_match = re.search(r"Sub\s*#\s*(\d+)", entry)
                    if not sub_match:
                        continue
        ...
        <p>Skipped picked-up / unmatched: {skipped}</p>
